    ...
    public class DestructionManager : MonoBehaviour
    {
        ...
        private void DestroyWithParticleEffect<T>(Destructible oldObj, ParticleSystem customParticle, T damageInfo)
        {
            if (oldObj.useFallbackParticle)
            {
                ...
                var rotation = oldObj.transform.rotation;
                ...
                if (oldObj.rotationOverride != Vector3.zero) {
                    rotation = Quaternion.Euler(oldObj.rotationOverride);
                }
                ...
                if (customParticle != null)
                {
                    ...
                    if (ParticleManager.Instance == null)
                        ObjectPool.Instance.Spawn(customParticle.gameObject, position, rotation);
                    else
                        ParticleManager.Instance.PlayEffect(customParticle, position, rotation, instanceId);
                }
